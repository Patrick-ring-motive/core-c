...
import {
  hasPropFunc
} from "../../vocabularies/code"
import {
  hasRef
} from "../../vocabularies/jtd/ref"
import {
  intRange,
  IntType
} from "../../vocabularies/jtd/type"
import {
  parseJson,
  parseJsonNumber,
  parseJsonString
...
export default function compileParser(
  ...
  const gen = new CodeGen(this.scope, {
    es5,
    lines,
    ownProperties
  })
...
function parseRef(cxt: ParseCxt): void {
  ...
  if (!hasRef(refSchema)) return parseCode({
    ...cxt,
    schema: refSchema
  })
  const {
  ...
  const sch = compileParser.call(self, new SchemaEnv({
    schema: refSchema,
    root
  }), definitions)
...
function parseWith(cxt: ParseCxt, parseFunc: {
  code: string
}, args ? : SafeExpr): void {
