...
const genSerialize: {
  [F in JTDForm]: (cxt: SerializeCxt) => void
} = {
...
function serializeRef(cxt: SerializeCxt): void {
  ...
  if (!hasRef(refSchema)) return serializeCode({
    ...cxt,
    schema: refSchema
  })
  const {
  ...
  const sch = compileSerializer.call(self, new SchemaEnv({
    schema: refSchema,
    root
  }), definitions)
