...
export type SomeJTDSchemaType = ( | // ref
  ...
  |
  {
    discriminator: string;mapping: Record < string,
    SomeJTDSchemaType >
  }
  // empty
  // NOTE see the end of
...
type IsEmptyRecord < T > = [T] extends [Record < string, never > ] ? [T] extends [never] ?
  false :
  true :
  false
...
export type JTDSchemaType < T, D extends Record < string, unknown > = Record < string, never >> = ( | // refs - where null wasn't specified, must match exactly
  ...
  ((true extends NullTypeEquality < T, number >
      ...
      ([RequiredKeys < Exclude < T, null >> ] extends [never] ? {
        ...
        properties: {
          [K in RequiredKeys < T > ]: JTDSchemaType < T[K],
          D >
        }
      }) &
      ([OptionalKeys < Exclude < T, null >> ] extends [never] ? {
        ...
        optionalProperties: {
          [K in OptionalKeys < T > ]: JTDSchemaType < Exclude < T[K],
          undefined > ,
          D >
        }
      }) & {
  ...
  definitions ? : {
    [K in keyof D]: JTDSchemaType < D[K],
    D >
  }
}
...
type JTDDataDef < S, D extends Record < string, unknown >> = | // ref
  (S extends {
    ...
    S extends {
      ...
      -readonly[K in keyof S["properties"]] - ? : JTDDataDef < S["properties"][K],
      D >
    } & {
    ...
    S extends {
      ...
      -readonly[K in keyof S["optionalProperties"]] + ? : JTDDataDef <
        S["optionalProperties"][K],
      D >
    } & ([S["additionalProperties"]] extends [true] ? Record < string, unknown > : unknown) : // values
    S extends {
      values: infer V
    } ?
    ...
    : // discriminator
    S extends {
      discriminator: infer M;mapping: Record < string,
      unknown >
    } ? [M] extends [string] ? {
      [K in keyof S["mapping"]]: JTDDataDef < S["mapping"][K],
      D > & {
        [KM in M]: K
      }
    } [keyof S["mapping"]] :
