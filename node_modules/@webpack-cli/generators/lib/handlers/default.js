...
async function questions(self, Question, config = {
  ...
  const {
    langType
  } = await Question.List(self, "langType", "Which of the following JS solutions do you want to use?", [config.langType.required ? "" : "none", "ES6", "Typescript"].filter((option) => option.length > 0), config.langType.required ? "ES6" : "none", self.force || config.langType.skip);
  switch (langType) {
    case "ES6":
  ...
  const {
    devServer
  } = await Question.Confirm(self, "devServer", "Do you want to use webpack-dev-server?", true, self.force || config.devServer.skip);
  if (devServer) {
  ...
  const {
    htmlWebpackPlugin
  } = await Question.Confirm(self, "htmlWebpackPlugin", "Do you want to simplify the creation of HTML files for your bundle?", true, self.force || config.htmlWebpackPlugin.skip);
  ...
  const {
    workboxWebpackPlugin
  } = await Question.Confirm(self, "workboxWebpackPlugin", "Do you want to add PWA support?", true, self.force || config.workboxWebpackPlugin.skip);
  if (workboxWebpackPlugin) {
    self.dependencies.push("workbox-webpack-plugin");
  }
  // Store all answers for generation
  self.answers = Object.assign(Object.assign({}, self.answers), {
    langType,
    devServer,
    htmlWebpackPlugin,
    workboxWebpackPlugin
  ...
  const {
    cssType
  } = await Question.List(self, "cssType", "Which of the following CSS solutions do you want to use?", [config.cssType.required ? "" : "none", "CSS only", "SASS", "LESS", "Stylus"].filter((option) => option.length > 0), config.cssType.required ? "CSS only" : "none", self.force || config.cssType.skip);
  if (cssType == "none") {
    self.answers = Object.assign(Object.assign({}, self.answers), {
      cssType,
      isCSS: false,
  ...
  const {
    isPostCSS
  } = await Question.Confirm(self, "isPostCSS", "Will you be using PostCSS in your project?", cssType == "CSS only", self.force);
  const {
    extractPlugin
  } = await Question.List(self, "extractPlugin", "Do you want to extract CSS for every file?", ["No", "Only for Production", "Yes"], "No", self.force);
  switch (cssType) {
    case "SASS":
...
function generate(self) {
  ...
  self.fs.copyTpl(resolveFile("webpack.configjs.tpl"), self.destinationPath("webpack.config.js"), Object.assign(Object.assign({}, self.answers), {
    entry
  }));
